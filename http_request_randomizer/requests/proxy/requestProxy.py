...
class RequestProxy:
    def __init__(self, web_proxy_list=[], sustain=False, timeout=5, protocol=Protocol.HTTPS, log_level=0):
        ...
        #####
        parsers = list([])
        parsers.append(FreeProxyParser('FreeProxy', 'http://free-proxy-list.net', timeout=timeout*5))
        ...
        parsers.append(PremProxyParser('PremProxy', 'https://premproxy.com', timeout=timeout*5))
        parsers.append(SslProxyParser('SslProxy', 'https://www.sslproxies.org', timeout=timeout*5))
        ...
        self.proxy_list = [p for p in self.proxy_list if protocol in p.protocols and ('US' or 'CA' in p.country.upper())]
        #for proxy in self.proxy_list:
            #print(proxy.protocols)

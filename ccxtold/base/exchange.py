...
class Exchange(object):
    ...
    def __init__(self, config={}):
        pac = get_pac(url='http://localhost/proxies.PAC')
        #self.session = None
        self.masterproxies = config['masterproxies']
        self.acount = 0
        #print(self.masterproxies)
        self.proxies = None
        self.session = PACSession(pac=pac, masterproxies=self.masterproxies)  # Session () by default
    ...
    def fetch(self, url, method='GET', headers=None, body=None):
        ...
        url = self.proxy + url
        request_headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36'
        ...
        try:
            if self.proxies == None or self.acount >= 80:
                self.proxies = self.session._proxy_resolver.get_proxy_for_requests(url)
                self.acount = 0
                sleep(0.1)
            ...
            #pac = get_pac(url='http://localhost/proxies.PAC')
            #self.session = None
            #self.session = PACSession(pac=pac, masterproxies=self.masterproxies)
            #self.session._tried_get_pac = False
            #pac = get_pac(url="http://localhost/proxies.PAC")
            #self = PACSession(pac=pac)  # In PY2, just saying 'raise' may re-raise ProxyConfigExhaustedError.

            #self.session._proxy_resolver = None
            #self.session.resetResolver(pac)

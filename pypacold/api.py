...
class PACSession(requests.Session):
    ...
    def __init__(self, pac=None, proxy_auth=None, pac_enabled=True,
        ...
        if pac:
            ...
            #pac = self.get_pac()
            self.get_pac(url='http://localhost/proxies.PAC')
            #self._proxy_resolver = self._get_proxy_resolver(self.pac)
    ...
    def request(self, method, url, proxies=None, **kwargs):
        ...
        :raises MalformedPacError: If something that claims to be a PAC file was downloaded but c   ould not be parsed.
        """
        ...
        if proxies == False:
            ...
            while True:
                ...
                try:
                    ...
                    #self.pac = self.get_pac(url='http://localhost/proxies.PAC')
                    self._tried_get_pac = False
                    self.get_pac(url='http://localhost/proxies.PAC')
                    print('exception in pypac request... retrying: ' + url)
                    print(request_exc)
                    return self.request(method, url, proxies=None, **kwargs)
...
def pac_context_for_url(url, proxy_auth=None):
    ...
    if pac:
        ...
        if proxies == False:
            ...
            print('7')
            self.get_pac(url='http://localhost/proxies.PAC')
            return pac_context_for_url(url, proxy_auth)

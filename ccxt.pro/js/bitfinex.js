...
module.exports = class bitfinex extends ccxt.bitfinex {
    ...
    async watchBalance (params = {}) {
        ...
        const future = this.authenticate ();
        const url = this.urls['api']['ws']['private'];
        const watching = this.afterDropped (future, this.watch, url, 'ws', undefined, 1);
        // purgeOrders here
        return await this.after (watching);
    }
    ...
    handleBalance(client, message) {
        id = this.safeString(message, 0)
        if (this.bals == undefined){
            this.bals = {}
        }
        symbols = []
        total = 0
        for (var bal in message[2]){
            abal = message[2][bal]
            if (abal[0] == 'trading'){
                try {
                    symbol = abal[1]
                    ...
                    */
                    ...
                    if (symbol.indexOf('USD') != -1){
                        total += amount
                        this.bals[symbol] = amount 
                    }
                    else if ( this.tickers.includes(symbol)){
                        console.log(this.tickers[symbol])
                        total += amount * ((this.tickers[symbol]['ask'] + this.tickers[symbol]['bid']) / 2)
                        this.bals[symbol] = amount * ((this.tickers[symbol]['ask'] + this.tickers[symbol]['bid']) / 2) 
                    }
                    this.bals['total'] = total
                }
                catch (err){
                    console.log(err)
                }
